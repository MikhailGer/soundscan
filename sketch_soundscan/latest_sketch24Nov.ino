...
//    Serial.println(currentTenzo);
    ...
    if (scan_in_progress) {
      if (base_run_flag){ // флаг разрешения движения двигателя базы
        if (!stepper_base.tick() && stepper_head.getCurrent() == circle_in_steps){
          return_base();  
                  
        }
        if (millis() - finding_timer >= serach_interval){
            return_base();  
         }
        }
...
//    stepper_base.setSpeed(speed_base);  // Устанавливаем скорость
  ...
  // /////////////////не тестировано////////////////////////////////////////
  else if (strcmp(command, "set_seraching_time") == 0){
    serach_interval = doc["searching_time"].as<int>();
    }

  else if(strcmp(command, "set_circle") == 0){
    circle_in_steps = doc["circle_in_steps"].as<int>();
    }
  else if(strcmp(command, "set_recording_time") == 0){
    prepearing_time = doc["recording_time"].as<int>();
    }
